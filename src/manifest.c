...
static int write_includes_plain(struct manifest *manifest)
{
	...
	string_or_die(&filename, "%s/%i/Manifest.%s.includes", conf,
		      manifest->version, manifest->component);
...
static int write_manifest_tar(struct manifest *manifest)
{
	...
	if (strcmp(manifest->component, "MoM") == 0) {
		string_or_die(&append, "%s Manifest.%s.includes", files, manifest->component);
		free(files);
		files = append;
	}
